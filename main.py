...
try: import requests, colorama, cursor; from capmonster_python import HCaptchaTaskProxyless
...
class Booster:
    ...
    def __name__(self) -> str:
        if self.config["use_basename"] == "true":
            print("basename")
            username = self.config["token base_name"] + " | " + "".join(random.choices("abcdefghijklmnopqrstuvwxyz", k=4))
            return username
        if self.config["use_basename"] == "false":
            print("usernames.txt")
    ...
    def __email__(self) -> str:
        ...
        email =  self.config["token base_name"] + "_" + "".join(random.choices("abcdefghijklmnopqrstuvwxyz", k=8)) + random.choice(domains)
        return email
    ...
    def __gen__(self, captcha_token: str, proxy: str) -> None:
        ...
        try:
            ...
            if 'captcha_key' in r.text:
                self.print(f'{self.red}Error   {self.purp}|{self.cyan} Invalid captcha')
                return
            
            if 'The resource is being rate limited.' in r.text:
                time_to_wait = int(r.json()['retry_after']) + 1
                self.print(f'{self.red}Error   {self.purp}|{self.cyan} ratelimited ' + proxy + " | " + str(time_to_wait) + 's')
            ...
            self.print(f'{self.green}Joined  {self.purp}|{self.cyan} ' + token)
            ...
            if captcha_token in self.captcha_tokens:
                ...
                proxy = random.choice(self.proxies)
                self.print(f'{self.red}Error   {self.purp}|{self.cyan} retrying with ' + proxy)
                self.captcha_tokens.append(captcha_token)
                self.__gen__(captcha_token, proxy)
    ...
    def __captcha__(self):
        while True:
            try:
                proxy = random.choice(self.proxies)
                capmonster = HCaptchaTaskProxyless(self.config["capmonster_key"]) 
                task_id = capmonster.createTask("https://discord.com/register", "4c672d35-0701-42b2-88c3-78380b0db560")
                self.print(f'{self.green}Task    {self.purp}|{self.cyan} ' + str(task_id))
                captcha_token = capmonster.joinTaskResult(int(task_id))
                self.print(f'{self.green}Solved  {self.purp}|{self.cyan} ' + (captcha_token[:15]))
